...
◾node.js의 런타임으로 사용됨
◾자바스크립트 IDE: Visual Studio Code 추천◾Microsoft Open Source IDE Project + 웹으로 개발됨 (크롬기반)
...
◾함수도 객체다. function이라는 객체의 인스턴스다. 다시 말해서 일종의 값이다. 배열의 값으로, 다른 함수의 인자로, 그리고 리턴값으로 사용할 수도 있다.
...
◾prototype 키워드를 사용하여 상속 가능◾객체 내 prototype 으로 정의된 프로퍼티 및 메소드를 새로운 객체가 prototype 으로 받아서 사용가능
◾자바스크립트 내장 객체들, 이 객체들에 prototype을 사용하여 유용한 객체확장 가능◦Object
◦Function
◦Array
◦String
◦Boolean
...
◾자바스크립트 원시 데이터 타입 (Primary Type), 이 외 모든 데이터 타입들은 객체다.◦숫자
◦문자열
◦불리언(true/false)
◦null
◦undefined
...
1.디버깅 원하는 페이지를 리로딩하고, Weinre 페이지(localhost:8080) 리로딩
2.Weinre 페이지 내 debug client user interface: http://localhost:8080/client/#anonymous 클릭
3.이제 디버깅 원하는 페이지를 크롬없이도 크롬 개발자 모드와 비슷하게 디버길 할 수 있음
...
이클립스 코드어시스트 설정:
이클립스는 기본적으로 JavaScript 자동완성(코드어시스트) 기능이 비활성화 되어있음
아래 두가지 설정변경 필요:
•첫째, Javascript 어시스트 기능 활성화:•Preferences → Javascript → Editor → Content Assist → Auto-Activation → Enable auto activation (첵크)
•둘째, Javascript 플러그인 설치•Help → Eclipse Marketplace → Nodeclipse.github.io 1.0.2 → Node.js Project 프로젝트 생성
...
•버전 v15부터 IE8 이하 버전을 지원하지 않음•페이스북은 이 버전부터 Production 에서 사용해도 된다고 안정성을 약속함
...
◾https://velopert.com/436 ← MongoDB 실행: C:\>mongod.exe --dbpath=".\data\db"
◾리액트 튜토리얼:
